...
public partial class VolumetricMesh : MeshInstance3D
{
	...
	public override void _Ready()
	{
		...
		// Scale the box so that the the larger dimension becomes 1.0
		Shader.SetShaderParameter("volume_shape", new Vector3(
			XResolution >= YResolution ? 1f : (float)XResolution / (float)YResolution,
			YResolution >= XResolution ? 1f : (float)YResolution / (float)XResolution,
			DepthScale));
